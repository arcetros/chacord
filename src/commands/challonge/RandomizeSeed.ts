...
export default class StartTournament extends Commands {
    ...
    execute = async (interaction: CommandInteraction): Promise<void> => {
        ...
        try {
            const response = await this.challonge.participant.randomize(tournament_id);

            interaction.editReply({ content: `Successfully randomized ${response.length} participants` });
        } catch (err) {
